...
(function(w){
    ...
    store.prototype = {
        ...
        setDataSet: function(obj, internal){
            var me = this,
                ...
                d = obj.data,
                dlen = d.length;
            console.log('entering setDataSet') ;
            ...
            console.log('colorizeeee hetamap') ;
            heatmap.colorize();
            this.set("data", d);
        },
    ...
    legend.prototype = {
        init: function(){
            ...
            // mic: added opacity
            element.style.cssText = "border-radius:5px;position:absolute;"+positionCss+"font-family:Helvetica; width:256px;z-index:10000000000; background:rgba(255,255,255,1);padding:10px;border:1px solid black;margin:0;opacity:0.70;-moz-opacity:0.70;filter:alpha(opacity=70);";
    ...
    heatmap.prototype = {
        ...
        colorize: function(x, y){
                ...
                var me = this;
                // commented below replace the radius based on radius object accd to datapoints
                var radius = me.get('radius')[x+y];
                ...
                for(var i=3; i < length; i+=4){
                    ...
                    // mic change this final alpha for lower weight
                    if (finalAlpha < 80) {
                        finalAlpha = 100;
                    }
        ...
        drawAlpha: function(x, y, count, colorize){
                ...
                // commented above replace the radius based on radius object accd to datapoints
                var radius = me.get('radius')[x+y];
                var  ctx = me.get("actx"),
                    max = me.get("max"),
                    bounds = me.get("bounds"),
                ...
                console.log('radius >>>');
                console.log(radius[x+'_'+y]);

                //ctx.shadowColor = ('rgba(0,0,0,'+((count)?(count/me.store.max):'0.1')+')');
                //replace above with this
                if(!count || (count/me.store.max) < 0.1) {
                    console.log((count/me.store.max)+ ' lessssssssss');
                    ctx.shadowColor = ('rgba(0,0,0,0.1)')
                }else{
                    ctx.shadowColor = ('rgba(0,0,0,'+(count/me.store.max)+')');
                }
                console.log(ctx.shadowColor+' --count--'+count);
